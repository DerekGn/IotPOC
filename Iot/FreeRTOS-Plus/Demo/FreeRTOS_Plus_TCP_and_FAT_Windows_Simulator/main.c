...
extern eHttpHandlerResult_t xApiProcessHttpRequest(HTTPClient_t *pxClient, BaseType_t xIndex);
extern eHttpHandlerResult_t xFileProcessHttpRequest(HTTPClient_t *pxClient, BaseType_t xIndex);
extern eHttpHandlerResult_t xSecurityProcessHttpRequest(HTTPClient_t *pxClient, BaseType_t xIndex);
...
static HttpHandler_t xApiHandler = { "Api", xApiProcessHttpRequest };
...
xConfigNextworkInterfaceToUse = atol(argv[1]);
	...
	***NOTE*** Tasks that use the network are created in the network event hook
	...
	#if( ( mainCREATE_FTP_SERVER == 1 ) || ( mainCREATE_HTTP_SERVER == 1 ) )
	{
		vAddHttpHandler( &xApiHandler );
		vAddHttpHandler( &xFileHandler );
